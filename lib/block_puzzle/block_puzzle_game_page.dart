...
class _BlockPuzzleGamePageState extends State<BlockPuzzleGamePage>
  ...
  // 드래그 상태
  int? _draggingIndex;
  Offset? _dragPosition; // 게임 영역 내 로컬 좌표
  int? _ghostX, _ghostY;
  ...
  static const List<Color> blockColors = [
    Color(0xFFFF6B6B), // 0: 레드
    Color(0xFFFECA57), // 1: 옐로우
    Color(0xFF48DBFB), // 2: 스카이블루
    Color(0xFF00D2D3), // 3: 틸
    Color(0xFFFF9FF3), // 4: 핑크
    Color(0xFF54A0FF), // 5: 블루
    Color(0xFF5F27CD), // 6: 퍼플
    Color(0xFF00B894), // 7: 그린
  ];
  ...
  void _calculateLayout(double width, double height) {
    ...
    const padding = 8.0;
    _gridPixelSize = min(width - 2 * padding, height * 0.65);
    ...
    for (int i = 0; i < 3; i++) {
      final x = padding + i * shapeAreaWidth;
      _shapeBounds.add(Rect.fromLTWH(x, _trayTop, shapeAreaWidth, _trayHeight));
    }
  }
  ...
  void _onPanStart(DragStartDetails details) {
    ...
    for (int i = 0;
        i < _shapeBounds.length && i < _engine.currentShapes.length;
        i++) {
  ...
  void _onPanEnd(DragEndDetails details) {
    ...
    if (_ghostValid && _ghostX != null && _ghostY != null) {
      final cleared = _engine.placeShape(_draggingIndex!, _ghostX!, _ghostY!);
      if (cleared > 0) {
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            // 최고 기록
            Text(
              '🏆 최고 기록: ${GameDataService.getBestScore('blockpuzzle')}',
            ...
            Row(
              ...
              children: [
                _buildStatItem('줄 클리어',
                    '${_engine.totalLinesCleared}줄', const Color(0xFF48DBFB)),
                _buildStatItem('최고 콤보', '×${_engine.maxCombo}',
                    ...
                    return GestureDetector(
                      ...
                      child: CustomPaint(
                        ...
                        painter: BlockPuzzlePainter(
                          ...
                          gridLeft: _gridLeft,
                          gridTop: _gridTop,
                          gridPixelSize: _gridPixelSize,
                          cellSize: _cellSize,
                          trayTop: _trayTop,
                          trayHeight: _trayHeight,
          ...
          Column(
            children: [
              const Text('콤보',
                  style: TextStyle(color: Colors.white54, fontSize: 11)),
              Text(
                _engine.combo > 0 ? '×${_engine.combo}' : '-',
                style: TextStyle(
                  color: _engine.combo > 0
                      ? const Color(0xFFFECA57)
                      : Colors.white38,
                  fontSize: 20,
                  fontWeight: FontWeight.bold,
          ...
          Column(
            children: [
              const Text('최고',
                  style: TextStyle(color: Colors.white54, fontSize: 11)),
              Text(
                '${GameDataService.getBestScore('blockpuzzle')}',
                style: const TextStyle(
...
class BlockPuzzlePainter extends CustomPainter {
  ...
  void _drawGrid(Canvas canvas) {
    final bgRect = RRect.fromRectAndRadius(
      Rect.fromLTWH(
          gridLeft - 2, gridTop - 2, gridPixelSize + 4, gridPixelSize + 4),
      ...
      for (int c = 0; c < BlockPuzzleEngine.gridSize; c++) {
        if ((r + c) % 2 == 0) {
          final rect = Rect.fromLTWH(
              gridLeft + c * cellSize, gridTop + r * cellSize,
              cellSize, cellSize);
          canvas.drawRect(
              rect, Paint()..color = Colors.white.withValues(alpha: 0.02));
        }
      }
    }
    ...
    for (int i = 0; i <= BlockPuzzleEngine.gridSize; i++) {
      ...
      canvas.drawLine(
          Offset(x, gridTop), Offset(x, gridTop + gridPixelSize), linePaint);
      canvas.drawLine(
          Offset(gridLeft, y), Offset(gridLeft + gridPixelSize, y), linePaint);
    }
  ...
  void _drawPlacedBlocks(Canvas canvas) {
    for (int r = 0; r < BlockPuzzleEngine.gridSize; r++) {
      for (int c = 0; c < BlockPuzzleEngine.gridSize; c++) {
        ...
        _drawBlock(canvas, gridLeft + c * cellSize, gridTop + r * cellSize,
            cellSize, blockColors[colorIdx], 1.0);
      }
    }
  }
  ...
  // ---- 개별 블록 그리기 ----
  void _drawBlock(
      Canvas canvas, double x, double y, double size, Color color, double alpha) {
    const gap = 1.5;
    ...
    if (alpha > 0.5) {
      canvas.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromLTWH(
              x + gap + 1, y + gap + 1, size - 2 * gap, size - 2 * gap),
          const Radius.circular(4),
        ),
    ...
    final highlightRect = RRect.fromRectAndRadius(
      Rect.fromLTWH(
          x + gap, y + gap, size - 2 * gap, (size - 2 * gap) * 0.35),
      const Radius.circular(4),
    );
  ...
  void _drawClearAnimation(Canvas canvas) {
    ...
    for (final key in engine.clearedCellKeys) {
      ...
      final halfSize = cellSize * scale / 2;
      ...
      canvas.drawRRect(
          rect, Paint()..color = Colors.white.withValues(alpha: alpha * 0.7));
    }
  }
  ...
  void _drawGhostPreview(Canvas canvas) {
    ...
    for (int r = 0; r < shape.height; r++) {
      for (int c = 0; c < shape.width; c++) {
        ...
        final gy = ghostY! + r;
        if (gx < 0 ||
            gx >= BlockPuzzleEngine.gridSize ||
            gy < 0 ||
            gy >= BlockPuzzleEngine.gridSize) continue;

        _drawBlock(canvas, gridLeft + gx * cellSize, gridTop + gy * cellSize,
            cellSize, color, ghostValid ? 0.4 : 0.15);
      }
    }

    // 유효한 위치면 글로우 테두리 표시
    if (ghostValid) {
      final shape2 = engine.currentShapes[draggingIndex!]!;
      // 고스트 영역의 바운딩 박스
      double minX = double.infinity, minY = double.infinity;
      double maxX = -double.infinity, maxY = -double.infinity;
      for (int r = 0; r < shape2.height; r++) {
        for (int c = 0; c < shape2.width; c++) {
          if (!shape2.cells[r][c]) continue;
      ...
      canvas.drawRRect(
        RRect.fromRectAndRadius(
            Rect.fromLTRB(minX, minY, maxX, maxY), const Radius.circular(4)),
        Paint()
          ..color = blockColors[shape2.colorIndex].withValues(alpha: 0.5)
          ..style = PaintingStyle.stroke
          ..strokeWidth = 2,
      );
    }
  ...
  void _drawShapeTray(Canvas canvas, Size size) {
    ...
    canvas.drawRRect(
        trayRect, Paint()..color = const Color(0xFF1A1A2E).withValues(alpha: 0.5));

    for (int i = 0;
        i < engine.currentShapes.length && i < shapeBounds.length;
        i++) {
      if (draggingIndex == i) continue; // 드래그 중인 건 트레이에 안 그림
      final shape = engine.currentShapes[i];
      if (shape == null) continue;

      // 놓을 수 없는 블록은 어둡게
      final canPlace = engine.canPlaceAnywhere(shape);
      _drawShapeInArea(canvas, shape, shapeBounds[i], canPlace ? 0.9 : 0.25);
    }
  }

  void _drawShapeInArea(
      Canvas canvas, BlockShape shape, Rect area, double alpha) {
    final maxDim = max(shape.width, shape.height);
    final sCellSize =
        min(area.width * 0.7, area.height * 0.75) / max(maxDim, 1);
    ...
    for (int r = 0; r < shape.height; r++) {
      for (int c = 0; c < shape.width; c++) {
        if (!shape.cells[r][c]) continue;
        _drawBlock(canvas, startX + c * sCellSize, startY + r * sCellSize,
            sCellSize, blockColors[shape.colorIndex], alpha);
      }
    }
  }
  ...
  void _drawDraggingShape(Canvas canvas) {
    ...
    for (int r = 0; r < shape.height; r++) {
      for (int c = 0; c < shape.width; c++) {
        if (!shape.cells[r][c]) continue;
        _drawBlock(canvas, startX + c * cellSize, startY + r * cellSize,
            cellSize, color, 0.85);
      }
    }
  }
  ...
  void _drawPopups(Canvas canvas, Size size) {
    for (final popup in engine.popups) {
      ...
      final style = TextStyle(
        color: popup.isCombo
            ? Color.lerp(const Color(0xFFFECA57), const Color(0xFFFF6B6B), 0.3)!
                .withValues(alpha: alpha)
            : Colors.white.withValues(alpha: alpha),
        fontSize: popup.isCombo ? 22 : 18,
        fontWeight: FontWeight.bold,
        shadows: [
          Shadow(
      ...
      final tp = TextPainter(
        text: TextSpan(text: popup.text, style: style),
